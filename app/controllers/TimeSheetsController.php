			...
			if ($task = ProjectTask::where('id',Input::get('task'))->first()):
				...
				if (in_array($task->user_id,$teamIDs)):
					...
					elseif(Input::get('run') == 1):
						...
						$task->stop_status = 0;
						$task->stop_date = '0000-00-00 00:00:00';
						ProjectTask::where('user_id',$task->user_id)->where('stop_status',0)->where('id','!=',$task->id)->update(['stop_status'=>1,'stop_date'=>date('Y-m-d H:i:s'),'updated_at'=>date('Y-m-d H:i:s')]);
					endif;
					$task->save();
					$task->touch();
				endif;
