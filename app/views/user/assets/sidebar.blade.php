...
<ul class="nav nav-sidebar">
    ...
    <li {{ Helper::isRoute('cooperators.index') }}>
        <?php
        $team = Team::where('excluded', 0)
                ->where(function ($query) {
                    $query->where('superior_id', Auth::user()->id)
                            ->orWhere('cooperator_id', Auth::user()->id);
                })->groupBy('superior_id')->groupBy('cooperator_id')->get()
        ?>

        <a href="{{ URL::route('cooperators.index') }}">Команда <span class="badge">{{ count($team) }}</span></a>
    </li>
